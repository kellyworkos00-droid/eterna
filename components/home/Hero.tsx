...
const Hero = () => {
  return (
    <section className="relative h-screen flex items-center justify-center texture-overlay bg-eterna-black overflow-hidden">
      {/* Background with subtle gradient */}
      <motion.div 
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 1.5 }}
        className="absolute inset-0 bg-gradient-to-br from-eterna-black via-eterna-maroon/10 to-eterna-black"
      ></motion.div>
      ...
      {/* Decorative Elements */}
      <motion.div 
        className="absolute top-20 left-10 w-32 h-32 border-2 border-eterna-maroon/20"
        animate={{ rotate: 45 }}
        transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
      />
      <motion.div 
        className="absolute bottom-20 right-10 w-40 h-40 border-2 border-eterna-maroon/10"
        animate={{ rotate: -360 }}
